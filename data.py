...
Two completely separate sources, as PPCap requires:

  CaptionData   (image features, factual caption)   -> factual model
  StyleData     (CLIP text embedding, styled text, style token) -> style model
...
class CaptionData(Dataset):
    ...
    def distinct_images(self, k):
        ...
        for img, _ in self.rows:
            if img in seen:
                continue
            seen.add(img)
            out.append(torch.load(os.path.join(self.cache, f"{img}.pt"),
    ...
    def __getitem__(self, i):
        ...
        e = self.tok(cap, max_length=self.max_len, truncation=True,
                     padding='max_length', return_tensors='pt')
        lab = e.input_ids.squeeze(0).clone()
        lab[lab == self.tok.pad_token_id] = -100
        return f.float(), e.input_ids.squeeze(0), e.attention_mask.squeeze(0), lab
...
class StyleData(Dataset):
    ...
    def __getitem__(self, i):
        ...
        e = self.tok(text, max_length=self.max_len, truncation=True,
                     padding='max_length', return_tensors='pt')
        return (emb, e.input_ids.squeeze(0), e.attention_mask.squeeze(0),
                torch.tensor(sid), torch.tensor(is_style))

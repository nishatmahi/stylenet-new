"""
sample.py — decode the test split at several style strengths.

    python sample.py                       # 20 images, quick look
    python sample.py --n_images 1000 --out_json preds.json    # full run
"""
...
def test_images(path, limit):
    ...
    FS-StyleCap and Detach-and-Attach.

    References are normalized with the same pipeline the model was trained
...
def main():
    ...
    settings = [("factual", 0.0)] + [(style, l) for l in lams]
    ...
    print(f"\nwrote {args.out_json} ({len(out)} images) — "
          f"BLEU/CIDEr on 'factual', human judging on the stylized fields")
